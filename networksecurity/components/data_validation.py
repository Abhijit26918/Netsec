...
class DataValidation:
    ...
    def detect_dataset_drift(self, base_df, current_df, threshold=0.05) -> bool:
        """
        KS test: for each column, compare the train (base) and test (current)
        distributions. If p-value < threshold, distributions differ significantly
        -> drift detected for that column, and overall validation status is False.
        """
        try:
            status = True
            report = {}
            for column in base_df.columns:
                ...
                drift_found = bool(threshold > is_same_dist.pvalue)
                if drift_found:
                    status = False
                report[column] = {
